/* ============================================
   CODACADE — Leaderboard Module (ES Module)
   ============================================ */
...
function addScore(gameId, username, score) {
    ...
    scores.push({ gameId, username, score, date: new Date().toISOString().split('T')[0] });
    saveScores(scores);
}
...
function renderLeaderboard(gameId, containerId) {
    ...
    const title = gameId.toUpperCase().replace(/-/g, ' ');
...
const leaderboard = { addScore, getTopScores, renderLeaderboard };
